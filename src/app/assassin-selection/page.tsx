...
//   playerName: string;
//   assassin: string;
//   // Add other properties if needed, e.g., cards, props
// }
...
const AssassinSelection = () => {
  ...
  const sessionName = searchParams.get('sessionName'); // Fetch session name from query params

  useEffect(() => {
    if (!sessionName) return;

    // Listen for changes to the players in real-time using onValue
    const sessionRef = ref(database, `gameSessions/${sessionName}`);
    
    const unsubscribe = onValue(sessionRef, (snapshot) => {
      const sessionData = snapshot.val();
      if (sessionData && sessionData.players) {
        setPlayers(sessionData.players);  // Update the players state with the latest data
      }
    });
  ...
  const handleAssassinSelection = async () => {
    ...
    try {
      ...
      if (existingPlayer) {
        ...
        // If player doesn't exist, add them to the session
        players.push({
          playerName,
  ...
  //     if (!sessionName) return; // If no session name, return early
  //     const sessionRef = ref(database, `gameSessions/${sessionName}`);
  //     const sessionSnapshot = await get(sessionRef);
  ...
  return (
    <div>
      ...
      <div className="flex justify-between max-w-7xl mx-auto px-8 py-6">
        <div
          className="w-1/3 p-4 rounded-lg text-ice-blue mr-8"
          style={{ backgroundColor: hoveredCharacter?.color || selectedCharacter?.color || "#505a5b" }}
        >
          ...
          {characters.map((character: Character, index: number) => (
            <div
              ...
              style={{
                ...
                outline: selectedCharacter?.name === character.name ? '4px solid #f3b481' : 'none'
              }}
            >
